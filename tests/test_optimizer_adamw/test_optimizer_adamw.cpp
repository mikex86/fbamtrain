...
int main()
{
    ...
    const Device device_gpu{DeviceType::GPU, 0};
    ...
    const fbamtrain::OptimizerConfiguration config{.type = "adamw",
    ...
    auto optimizer = fbamtrain::optim::CreateOptimizer(config, parameters, device_gpu, compute_stream_descriptor);
    optimizer->step();

    for (size_t i = 0; i < parameters.size(); ++i)
    {
        testing::AssertSimilar(expected[i], parameters[i].param, 1e-6);
    }
    return 0;
}
