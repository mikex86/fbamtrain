...
namespace
{
    ...
    {
        auto expected = RealTensor::Allocate({M, N}, DataType::BFLOAT16, DEVICE_CPU);
        auto *out_data = static_cast<uint16_t *>(expected->dataptr());
        ...
        for (uint64_t row = 0; row < M; ++row)
        {
            for (uint64_t col = 0; col < N; ++col)
            {
                ...
                out_data[row * N + col] = utils::Bf16FromFp32(acc);
            }
        }
...
int main()
{
    ...
    TraceTensor out_gpu = graph.createTensor({M, N}, DataType::BFLOAT16, DEVICE_GPU, main_stream_desc, false);
    TraceTensor out_cpu = graph.createTensor({M, N}, DataType::BFLOAT16, DEVICE_CPU, main_stream_desc, false);
    ...
    const std::string expected_kernel_substr = PreferCutlassGemm() ? "cutlass_matmul_bf16_kernel"
                                                                    : "matmul_bf16_cstore";
