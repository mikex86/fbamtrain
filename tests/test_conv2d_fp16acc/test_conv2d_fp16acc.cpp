...
int main()
{
    ...
    pi::tensorlib::TraceTensor input =
        init_graph.createTensor({BATCH, IN_CHANNELS, HEIGHT, WIDTH}, pi::tensorlib::DataType::FLOAT16, device,
    ...
    struct pi::tensorlib::Conv2d conv("conv", IN_CHANNELS, OUT_CHANNELS, KERNEL_SIZE, STRIDE, PADDING, DILATION, device,
                                      pi::tensorlib::DataType::FLOAT16, init_graph, init_seed, main_stream_desc, false,
                                      true);
    ...
    pi::tensorlib::safetensors::SaveToFile("actual_debug.safetensors", {{"output", actual_output}});
