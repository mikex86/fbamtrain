...
def generate_reference(dtype: torch.dtype, suffix: str):
    ...
    bias_trailing = torch.zeros(COLS, dtype=dtype, device="cuda")

    seed = 123
    fill_uniform_(activation, -0.5, 0.5, seed=seed)
    seed += 1
    fill_uniform_(bias_trailing, -0.5, 0.5, seed=seed)

    output_trailing = activation + bias_trailing

    save_file({
        "output": output_trailing.cpu(),
