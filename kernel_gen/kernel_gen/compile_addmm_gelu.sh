...
run_cutlass_pack() {
  ...
  ${CUTLASS_RUNNER} compile.py \
      --gemm-config addmm_gelu_cutlass_bf16 \
      --gemm-config addmm_gelu_cutlass_fp16 \
      --gemm-config addmm_gelu_cutlass_fp16_acc_fp16 \
      --gemm-config addmm_gelu_preact_cutlass_bf16 \
      --gemm-config addmm_gelu_preact_cutlass_fp16 \
      --gemm-config addmm_gelu_preact_cutlass_fp16_acc_fp16 \
      --autotune-config "${autotune_cfg}" \
      --package --sm "${sm}"

  ${CUTLASS_RUNNER} compile.py \
      --gemm-config matmul_gelu_cutlass_bf16 \
      --gemm-config matmul_gelu_cutlass_fp16 \
      --gemm-config matmul_gelu_cutlass_fp16_acc_fp16 \
      --gemm-config matmul_gelu_bwd_cutlass_bf16 \
      --gemm-config matmul_gelu_bwd_cutlass_bf16_ta \
      --gemm-config matmul_gelu_bwd_cutlass_bf16_tb \
      --gemm-config matmul_gelu_bwd_cutlass_bf16_tab \
      --gemm-config matmul_gelu_bwd_cutlass_fp16 \
      --gemm-config matmul_gelu_bwd_cutlass_fp16_ta \
      --gemm-config matmul_gelu_bwd_cutlass_fp16_tb \
      --gemm-config matmul_gelu_bwd_cutlass_fp16_tab \
      --gemm-config matmul_gelu_bwd_cutlass_fp16_acc_fp16 \
      --gemm-config matmul_gelu_bwd_cutlass_fp16_acc_fp16_ta \
      --gemm-config matmul_gelu_bwd_cutlass_fp16_acc_fp16_tb \
      --gemm-config matmul_gelu_bwd_cutlass_fp16_acc_fp16_tab \
      --autotune-config "${autotune_cfg}" \
      --package --sm "${sm}"
}
