...
using Traits = fbamtrain::kernel_gen::cutlass3_conv2d::WgradTraits<
    CUTLASS_CONV_SM_ARCH, Element, ElementAccumulator, CUTLASS_CONV_THREADBLOCK_M, CUTLASS_CONV_THREADBLOCK_N,
    CUTLASS_CONV_THREADBLOCK_K, CUTLASS_CONV_CLUSTER_M, CUTLASS_CONV_CLUSTER_N, CUTLASS_CONV_CLUSTER_K,
    CUTLASS_CONV_ALIGNMENT_A, CUTLASS_CONV_ALIGNMENT_B>;

using ConvKernel = typename Traits::ConvKernel;
using Params = typename Traits::Params;

struct alignas(alignof(Params)) WgradKernelParams
{
    unsigned char storage[sizeof(Params)];
};

static_assert(sizeof(WgradKernelParams) == sizeof(Params));
static_assert(alignof(WgradKernelParams) >= alignof(Params));
} // namespace
...
#endif
        void CUTLASS_CONV_KERNEL_NAME(CUTLASS_GRID_CONSTANT WgradKernelParams const raw_params)
{
