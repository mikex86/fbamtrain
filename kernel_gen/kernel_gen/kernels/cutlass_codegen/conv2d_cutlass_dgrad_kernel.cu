...
using ConvOperation = cutlass::conv::device::ImplicitGemmConvolution<Conv2dDgradKernel>;
using Params = typename Conv2dDgradKernel::Params;
...
CUTLASS_DEVICE ConvOperation::Arguments make_arguments(
  ...
  typename Conv2dDgradKernel::TensorRefA ref_A(mutable_a, LayoutInputA::packed(output_extent));
  typename Conv2dDgradKernel::TensorRefB ref_B(mutable_b, LayoutInputB::packed(weight_extent));

  typename Conv2dDgradKernel::TensorRefC ref_C(nullptr, LayoutOutput::packed(input_extent));
  typename Conv2dDgradKernel::TensorRefC ref_D(ptr_d, LayoutOutput::packed(input_extent));
...
__device__ __constant__ int cutlass_conv2d_shared_mem_bytes = sizeof(typename Conv2dDgradKernel::SharedStorage);
...
extern "C" __global__ void CUTLASS_CONV_KERNEL_NAME(
    ...
    ElementOutput *ptr_dinput,
    ElementCompute alpha,
    ElementCompute beta,
    int *workspace) {
  extern __shared__ int SharedStorageBase[];
  auto *shared_storage = reinterpret_cast<typename Conv2dDgradKernel::SharedStorage *>(SharedStorageBase);
