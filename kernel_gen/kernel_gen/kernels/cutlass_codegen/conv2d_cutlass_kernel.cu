...
using ConvOperation = cutlass::conv::device::ImplicitGemmConvolution<Conv2dFpropKernel>;
using Params = typename Conv2dFpropKernel::Params;
...
CUTLASS_DEVICE ConvOperation::Arguments make_arguments(
  ...
  typename Conv2dFpropKernel::TensorRefA ref_A(mutable_a, LayoutInputA::packed(input_extent));
  typename Conv2dFpropKernel::TensorRefB ref_B(mutable_b, LayoutInputB::packed(weight_extent));

  typename Conv2dFpropKernel::TensorRefC ref_C(nullptr, LayoutOutput::packed(output_extent));
  typename Conv2dFpropKernel::TensorRefC ref_D(ptr_d, LayoutOutput::packed(output_extent));
...
__device__ __constant__ int cutlass_conv2d_shared_mem_bytes = sizeof(typename Conv2dFpropKernel::SharedStorage);
...
extern "C" __global__ void CUTLASS_CONV_KERNEL_NAME(
    ...
    ElementInputA const *ptr_input,
    ElementInputB const *ptr_weight,
    ElementOutput *ptr_output,
    ElementCompute alpha,
    ElementCompute beta,
    int *workspace) {
  extern __shared__ int SharedStorageBase[];
  auto *shared_storage = reinterpret_cast<typename Conv2dFpropKernel::SharedStorage *>(SharedStorageBase);
