...
def add_trailing_broadcast(
    ...
    bias = tl.load(bias_ptr + tile_cols * stride_bias, mask=mask, other=0)
...
@triton.autotune(configs=_ELEMENTWISE_CONFIGS, key=["n_elements"])
@triton.jit
def add_elementwise_fp32_to_fp16(
    out_ptr,
    lhs_ptr,
    rhs_ptr,
    n_elements,
...
def launch_add_trailing_broadcast(
    x: torch.Tensor,
    bias: torch.Tensor,
    ...
    *,
    ...
    assert bias.ndim == 1, "expected 1D bias"
    if x.dtype != bias.dtype or x.dtype != out.dtype:
        raise ValueError("launch_add_trailing_broadcast expects matching dtypes")

    rows, cols = x.shape
    assert bias.numel() == cols, "bias must match trailing dimension"
    stride_row, stride_col = x.stride()
    stride_bias = bias.stride()[0]

    meta = {"BLOCK_SIZE": block_size} if block_size is not None else None
    kernel = _resolve_kernel(add_trailing_broadcast, meta)
...
def launch_add_elementwise(
    ...
    *,
    ...
    """Utility to run the element-wise add kernel in eager mode for debugging."""
    if out is None:
    ...
    if lhs.dtype != rhs.dtype or lhs.dtype != out.dtype:
        raise ValueError("launch_add_elementwise expects matching dtypes")

    n_elements = lhs.numel()

    meta = {"BLOCK_SIZE": block_size} if block_size is not None else None
    kernel = _resolve_kernel(add_elementwise, meta)
...
def benchmark_add_trailing_broadcast(rows=2048, cols=512, iters=100, block_size: int | None = None):
    device = torch.device("cuda")
    dtype = torch.float16

    torch.manual_seed(0)
    activation = torch.randn((rows, cols), device=device, dtype=dtype)
    bias = torch.randn((rows,), device=device, dtype=dtype)

    out_triton = torch.empty_like(activation)

    def run_triton():
        launch_add_trailing_broadcast(activation, bias, out_triton, block_size=block_size)
        return out_triton

    def run_torch():
        return activation + bias.unsqueeze(1)
    ...
    elems = rows * cols
    bytes_moved = elems * dtype.itemsize * 3  # read activation + bias (broadcast) + write output (approx)
    ...
    print(f"Add Trailing Broadcast rows={rows}, cols={cols}, block={block_desc}, dtype={dtype}")
...
def benchmark_add_elementwise(numel=2048 * 512, iters=100, block_size: int | None = None):
    device = torch.device("cuda")
    dtype = torch.float16
    ...
    def run_triton():
        launch_add_elementwise(lhs, rhs, out_triton, block_size=block_size)
        return out_triton
    ...
    block_desc = block_size if block_size is not None else "auto"
    print(f"Add Elementwise FP16 numel={numel}, block={block_desc}")
    print(f" Triton: {triton_ms:.3f} ms  ({gbps_triton:.2f} GB/s)")
    print(f" Torch : {torch_ms:.3f} ms  ({gbps_torch:.2f} GB/s)")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Benchmark Triton add kernels against PyTorch")
    parser.add_argument(
        "--mode",
    ...
    parser.add_argument("--numel", type=int, default=None, help="Number of elements for elementwise add benchmarking")
    ...
    if args.mode == "trailing-broadcast":
        benchmark_add_trailing_broadcast(
            rows=args.rows,
            cols=args.cols,
            iters=args.iters,
